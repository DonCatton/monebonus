...
if($req_uri != '/usr' AND !preg_match("|^[/usr?p=]+[0-9]{1,5}$|u",$req_uri)){Header("Location: /usr");die();}
@require_once("_add/_header.php");
if(isset($_SESSION["payeer"])){@require_once("_add/_menu.php");}
?>
								<h1>Пользователи</h1>
<p>Сортировка по: &nbsp;<b>ID</b> / <a href="/usrs"><font color="#838383">Балансу</font></a></p><br>
...
$bd->Query("SELECT `payeer`,`balance`,`date_reg`,`bns_kol` FROM `users_nykfageubf` ORDER BY `id` DESC LIMIT ".$start.",".$elems);
